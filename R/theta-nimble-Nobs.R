...
p_obs <- matrix(1, nrow = dim(N)[1], ncol = dim(N)[2])
...
predict_r_mn_nimble <- nimbleCode({
  ...
  # If missing years in N are 0
  for (i in 1:pops){

    for (t in 1:(tmax-1)){

      log(N[i, t+1]) <- log(N[i, t]) + obs_r[i, t]

      N_obs[i, t] ~ dnorm(N[i, t] * p_obs[i, t], sd = sigma_obs[i] * p_obs[i, t])

    }

    sigma_obs[i] ~ dunif(0, 100)

  }

  # If missing years in N are NA
  # for (i in 1:pops){
  #
  #   for (t in 1:(tmax-1)){
  #
  #     log(N[i, t+1]) <- log(N[i, t]) + obs_r[i, t]
  #
  #     N_obs[i, t] ~ dnorm(N[i, t], sd = sigma_N[i])
  #
  #   }
  #
  #   sigma_N[i] ~ dunif(0, 100)
  #
  # }
...
sample_inits_b5 <- function(){
  ...
  list(
    ...
    K = rep(K, pops),
    sigma_obs = runif(pops, 0, 10)
    # N_obs = matrix with initial values for missing years and NA for non-missing years
  )

}

#inits_b5 <- list(sample_inits_b5(), sample_inits_b5(), sample_inits_b5())
inits_b5 <- list(sample_inits_b5())
...
input_data_b5 <- list(N_obs = round(N), p_obs = p_obs, obs_r = obs_r)

input_constants_b5 <- list(tmax = tmax, max_K = rep(K*2, pops), sigma_d2 = rep(sigma_d2, pops))

## Set parameters to monitor
params_b5 <- c("K", "theta", "sigma_e2", "mu_r1", "gamma", "sigma_obs")
...
#start <- Sys.time()
mod_b5 <- nimbleMCMC(code = predict_r_mn_nimble,
                     constants = input_constants_b5,
                     data = input_data_b5,
                     inits = inits_b5,
                     monitors = params_b5,
                     niter = niter,
